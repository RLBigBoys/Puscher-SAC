...
This project studies continuous-control reinforcement learning on `Pusher-v5` with Soft Actor-Critic (SAC). The agent controls a 7-DoF robotic arm and must push an object to a target location. Success is defined as a final object-goal distance below a configurable threshold.
...
SAC is an off-policy actor-critic algorithm. The actor learns a stochastic policy, the critics learn soft Q-functions, and a replay buffer stores past transitions for sample-efficient updates. In this repository the policy and value networks are multi-layer perceptrons with configurable hidden sizes.
...
1. Create the environment and SAC model.
...
After training, place the numerical results from `eval.json` here and reference the generated figures. A typical baseline report should discuss whether the reward curve trends upward, whether the final distance decreases, and whether the success rate becomes non-zero.
